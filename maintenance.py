...
async def maintenance_gatekeeper(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.message:
        # User ko Current Message bhejo
        await update.message.reply_text(f"**🚧 {CURRENT_MSG}**", parse_mode="Markdown")
    
    elif update.callback_query:
        await update.callback_query.answer("🚧 Maintenance Mode is ON!", show_alert=True)
...
async def maintenance_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        await update.message.reply_text(
            "⚠️ **Usage:**\n"
            "`/maintenance on` (Default Msg)\n"
            "`/maintenance on Server Update` (Custom Msg)\n"
            "`/maintenance off`"
        )
        return
    ...
    if action == "on":
        ...
        # Check karo agar user ne msg diya hai (Example: /maintenance on Hello World)
        if len(context.args) > 1:
            raw_text = " ".join(context.args[1:]) # 'Hello World' ban jayega
            CURRENT_MSG = make_small_caps(raw_text) # Small Caps convert
        else:
            CURRENT_MSG = DEFAULT_MSG # Default wapis set karo
            
        # Database me save karo
        await set_maintenance(True, CURRENT_MSG)
        
        await update.message.reply_text(f"✅ **Maintenance ON!**\n\n💬 **Message:**\n{CURRENT_MSG}")
